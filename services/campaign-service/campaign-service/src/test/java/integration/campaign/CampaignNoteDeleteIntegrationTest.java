...
class CampaignNoteDeleteIntegrationTest {
    ...
    void shouldDeleteNote() throws JsonProcessingException {
        // given
        var campaign = createCampaign("Note Delete");
        ...
        .when()
                .delete("/campaigns/{cid}/notes/{nid}", campaign.id(), noteId)
        .then()
                .statusCode(204);
    }
    ...
    @Test
    @TestSecurity(user = "1", roles = "PLAYER")
    void shouldFailWhenDeletingNonexistentNote() {
        // when / then
        given()
                .queryParam("userId", 1) // hardcoded: matches @TestSecurity user
        .when()
    ...
    void shouldFailWhenNotAuthenticated() {
        ...
        .when()
                .delete("/campaigns/{cid}/notes/{nid}", 1L, 1L) // hardcoded: arbitrary, auth fails first
        .then()
                .statusCode(401);
    }
