...
class CampaignQuestFindByIdIntegrationTest {
    ...
    void shouldFindQuestById() {
        var campaign = given()
            ...
            .body("""
                {"userId":1,"name":"Quest Find","description":"desc","maxPlayers":4}
                """)
        .when()
            .post("/campaigns")
        .then()
        ...
        var questId = given()
            .contentType(ContentType.JSON)
            .body("""
                {"userId":1,"title":"Find Me","description":"desc","status":"ACTIVE","priority":"MAIN"}
                """)
        .when()
        ...
        given()
            .queryParam("userId", 1)
        .when()
            .get("/campaigns/" + campaign.id() + "/quests/" + questId)
        .then()
            .statusCode(200)
            .contentType(ContentType.JSON)
            .body("id", org.hamcrest.Matchers.equalTo((int) questId))
            .body("title", org.hamcrest.Matchers.equalTo("Find Me"));
    }
    ...
    @DeleteEntities(from = CampaignQuestEntity.class)
    @DeleteEntities(from = CampaignEntity.class)
    void shouldReturn404WhenQuestNotFound() {
        var campaign = given()
            .contentType(ContentType.JSON)
            .body("""
        ...
        given()
            .queryParam("userId", 1)
        .when()
            .get("/campaigns/" + campaign.id() + "/quests/999999")
        .then()
            .statusCode(404);
    }
