package com.dndplatform.compendium.client.characterclass;
...
@RequestScoped
public interface CharacterClassFindByIdResourceRestClient extends CharacterClassFindByIdResource {

    @Override
    @GET
    @Path("/{id}")
    @Retry(delay = 100, delayUnit = ChronoUnit.MILLIS)
    CharacterClassViewModel findById(@PathParam("id") int id);
}
