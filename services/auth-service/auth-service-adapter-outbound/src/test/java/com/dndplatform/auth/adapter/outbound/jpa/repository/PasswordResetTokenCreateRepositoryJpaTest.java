...
class PasswordResetTokenCreateRepositoryJpaTest {

    @Mock
    private PasswordResetPanacheRepository panacheRepository;

    private PasswordResetTokenCreateRepositoryJpa sut;

    @BeforeEach
    void setUp() {
        sut = new PasswordResetTokenCreateRepositoryJpa(panacheRepository, 15);
    }

    @Test
    void shouldPersistEntityAndReturnTokenWithPlaintextOtpCode(@Random long userId) {
        willDoNothing().given(panacheRepository).persist(any(PasswordResetEntity.class));
