...
public class EncounterCompleteResourceImpl implements EncounterCompleteResource {

    private final EncounterCompleteDelegate delegate;

    @Inject
    public EncounterCompleteResourceImpl(@Delegate EncounterCompleteResource delegate) {
        this.delegate = (EncounterCompleteDelegate) delegate;
    }

    @POST
    @Path("/{id}/complete")
    @Override
    @Operation(summary = "Complete encounter", description = "Mark an active encounter as completed")
    @APIResponses({
            @APIResponse(
                    responseCode = "200",
